...
public class IngressBackupMplsInstaller
        implements IntentInstaller<IngressMplsBackupPathIntent> {
    ...
    public void activate() {
        ...
        intentManager
                .registerInstaller(IngressMplsBackupPathIntent.class, this);
    }

    @Deactivate
    public void deactivate() {
        intentManager.unregisterInstaller(IngressMplsBackupPathIntent.class);
    }

    @Override
    public List<Collection<FlowRuleOperation>> install(IngressMplsBackupPathIntent intent) {
    ...
    @Override
    public List<Collection<FlowRuleOperation>> uninstall(IngressMplsBackupPathIntent intent) {
    ...
    @Override
    public List<Collection<FlowRuleOperation>> replace(IngressMplsBackupPathIntent oldIntent,
                                                       IngressMplsBackupPathIntent newIntent) {
    ...
    private LinkResourceAllocations assignMplsLabel(IngressMplsBackupPathIntent intent) {
        ...
        log.info("Path: {} - ingress label: {} - egress label: {}", intent
                .path().links(), intent.ingressLabel(), intent.egressLabel());
    ...
    private List<Collection<FlowRuleOperation>> generateRules(IngressMplsBackupPathIntent intent,
    ...
    private FlowRuleOperation singleFlow(PortNumber inPort, Link link,
        ...
        if (intent.ingressLabel().isPresent()) {
            ...
            // if the flow has not MPLS label and we do not need to add a label
            // (egress is null), we simply forward the traffic
        }
    ...
    private Collection<FlowRuleOperation> egressFlow(PortNumber inPort,
                                                     Link link,
                                                     IngressMplsBackupPathIntent intent,
                                                     MplsLabel prevLabel,
                                                     FlowRuleOperation.Type operation) {
        ...
        if (intent.egressLabel().isPresent()) {
            ...
            log.warn("Previous assigned label does not exist!");
        }

        // Match on the previous label is done on the table 2.
        treat.transition(FlowRule.Type.MPLS);

        Collection<FlowRuleOperation> flowRuleOperations = new ArrayList<FlowRuleOperation>();

        flowRuleOperations.add(flowRuleOperation(intent, link.src().deviceId(),
                                                 selector.build(),
                                                 treat.build(), operation));

        TrafficSelector.Builder selecEgress = DefaultTrafficSelector.builder();
        intent.egressLabel().ifPresent(label -> selecEgress.matchEthType(Ethernet.MPLS_UNICAST)
    ...
    protected FlowRuleOperation flowRuleOperation(MplsPathIntent intent,
                                                  ...
                                                  FlowRuleOperation.Type operation) {
        FlowRule rule = new DefaultFlowRule(deviceId, selector, treat, 123, // FIXME
                                                                            // 123
                                            appId, 0, true);
